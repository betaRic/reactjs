...
import { deleteFacebookConnection, FACEBOOK_SESSION_COOKIE, getFacebookConnections, selectFacebookPage } from "@/lib/facebook-connections";
...
export async function POST(request) {
  try {
    ...
    await selectFacebookPage(request, pageId);
    return Response.json({ ok: true, ...(await getFacebookConnections(request)) });
  } catch (error) {
    return toRouteError(error);
  }
